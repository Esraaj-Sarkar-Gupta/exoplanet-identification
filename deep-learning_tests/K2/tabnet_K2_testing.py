...
def main():
    # Use the IMPUTED K2 dataset -- Neural Networks cannot process NaNs
    data_path = PROJECT_ROOT / "data" / "k2" / "k2pandc_imputed.csv"
    print(f"Loading Imputed K2 data from: {data_path.name}...")
    
    df_imputed = pd.read_csv(data_path)
    df_imputed.dropna(subset=['Target'], inplace=True)
    
    print("Computing physics-aware features...")
    df_engineered = engineer_physics_features(df_imputed)
